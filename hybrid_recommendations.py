...
class HybridRecommenderSystem:
    ...
    def __normalize_similarities(self, similarity_scores):
        similarity_scores = np.array(similarity_scores).ravel().astype(float)
    ...
    def __weighted_combination(self, content_based_scores, collaborative_filtering_scores):
        ...
        cb = np.array(content_based_scores).ravel()
        cf = np.array(collaborative_filtering_scores).ravel()

        # ensure same length
        min_len = min(len(cb), len(cf))
        cb = cb[:min_len]
        cf = cf[:min_len]

        # apply weights
        scores = (self.weight_content_based * cb) + (self.weight_collaborative * cf)
    ...
    #     weighted_scores = (
    #         self.weight_content_based * cb +
    #         self.weight_collaborative * cf
    #     )
    ...
    # def __weighted_combination(self, content_based_scores, collaborative_filtering_scores):
        ...
        min_len = min(content_based_scores.shape[1], collaborative_filtering_scores.shape[1])

        content_based_scores = content_based_scores[:, :min_len]
        collaborative_filtering_scores = collaborative_filtering_scores[:, :min_len]
    ...
    def give_recommendations(self, song_name, artist_name, songs_data, track_ids, transformed_matrix, interaction_matrix):
        ...
        cb = np.array(normalized_content_based_similarities).ravel() ** 1.5
        cf = np.array(normalized_collaborative_filtering_similarities).ravel() ** 1.5


        weighted_scores = self.__weighted_combination(
            content_based_scores=cb,
            collaborative_filtering_scores=cf
        )
        ...
        cb = normalized_content_based_similarities
        cf = normalized_collaborative_filtering_similarities

        print("CB range:", cb.min(), cb.max())
        print("CF range:", cf.min(), cf.max())
